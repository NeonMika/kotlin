...
class JsIrBackendContext(
    ...
    fun getJsCodeForFunction(symbol: IrFunctionSymbol): JsFunction? {
        ...
        if (!sourceMap.isNullOrEmpty()) {
            // This annotation was generated by JsCodeOutliningLowering
            val parsedSourceMap = when (val result = SourceMapParser.parse(sourceMap)) {
                is SourceMapSuccess -> result.value
                is SourceMapError -> compilationException("Invalid source map in @JsFun annotation. ${result.message}", jsFunAnnotation)
            }
